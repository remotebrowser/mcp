...
class Element:
    ...
    async def is_visible(self) -> bool:
        """
        Optimized visibility check using a single, efficient JavaScript evaluation.
        Uses the selector to find element and checks visibility in one pass.
        """
        try:
...
async def page_query_selector(
    ...
    """
    Query selector with optional visibility check skip.

    Args:
        skip_visibility_check: If True, skip visibility check (use when already verified via batch check)
    """
    try:
