"""MCP Apps support — typed UI metadata models for tools and resources.

Provides Pydantic models for the MCP Apps extension (io.modelcontextprotocol/ui),
enabling tools and resources to carry UI metadata for clients that support
...
class ResourcePermissions(BaseModel):
    ...
    camera: dict[str, Any] | None = Field(default=None, description="Request camera access")
    microphone: dict[str, Any] | None = Field(default=None, description="Request microphone access")
...
class ToolUI(BaseModel):
    """Typed ``meta["ui"]`` for tools — links a tool to its UI resource.
...
class ResourceUI(BaseModel):
    """Typed ``meta["ui"]`` for resources — rendering hints for UI-capable clients."""
